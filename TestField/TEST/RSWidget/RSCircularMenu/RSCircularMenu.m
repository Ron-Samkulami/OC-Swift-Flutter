...
// github: https://github.com/linshaolie
...
#pragma mark - RS_centerButton
@interface RS_centerButton : UIButton

@property (nonatomic, strong) UIColor* normalColor;
@property (nonatomic, strong) UIColor* selectedColor;
@property (nonatomic, strong) UIImage* centerIcon;
@property (nonatomic, strong) UIImageView* centerIconView;
@property (nonatomic, assign) RSIconType type;

- (instancetype)initWithFrame:(CGRect)frame type:(RSIconType)type;

@end


#pragma mark - RS_roundCircle
@interface RS_roundCircle : UIView <UIGestureRecognizerDelegate>
/// 触摸点
...
#pragma mark - RSCircularMenu - IMP

@interface RSCircularMenu ()

@property (nonatomic, strong) RS_centerButton * centerButton;
@property (nonatomic, strong) RS_roundCircle * roundCircle;
@property (nonatomic, assign) CGFloat innerCircleRadius;
@property (nonatomic, strong) NSMutableArray* icons;

- (void)drawCenterIconInRect:(CGRect)rect state:(UIControlState)state;

@end

@implementation RSCircularMenu
...
- (void)setup{
    ...
    self.centerIconType = RSIconTypePlus;
...
- (RS_centerButton *)centerButton
{
    if (!_centerButton) {
        _centerButton = [[RS_centerButton alloc] initWithFrame:CGRectMake(0, 0, self.centerButtonSize.width, self.centerButtonSize.height) type:self.centerIconType];
...
- (RS_roundCircle *)roundCircle
{
    if (!_roundCircle) {
        _roundCircle = [[RS_roundCircle alloc] initWithFrame:CGRectZero];
...
- (void)setCenterIconType:(RSIconType)centerIconType
{
...
#pragma mark - RS_centerButton - IMP
@implementation RS_centerButton
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (self.type == RSIconTypePlus || self.state == UIControlStateSelected) {
        //// Rectangle Drawing
    ...
    else if (self.type == RSIconTypeUserDraw)
    {
        if ([self.superview respondsToSelector:@selector(drawCenterIconInRect:state:)]) {
            [(id)self.superview drawCenterIconInRect:rect state:self.state];
        }
    }
    else if (self.type == RSIconTypeCustomImage){
...
- (void)setSelected:(BOOL)selected
{
    ...
    //中心图片转动动画 usingSpringWithDamping数值越小，弹簧效果更明显
    [UIView animateWithDuration:0.24
                          delay:0
...
#pragma mark - RS_roundCircle - IMP
@implementation RS_roundCircle
